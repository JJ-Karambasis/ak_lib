...
set compile_common=-I%BasePath% -I%ndk_native_app_blue_path% %sysroot% -DANDROID_BUILD -DANDROID -fdiagnostics-absolute-paths -fPIC -std=c89 %compile_warnings%
...
pushd %lib_path%\armeabi-v7a
    call %ndk_clang_path%\armv7a-linux-androideabi31-clang %compile_flags% -c -Wno-comment %ndk_native_app_blue_path%\android_native_app_glue.c %compile_out% android_native_app_glue.o
    call %ndk_clang_path%\armv7a-linux-androideabi31-clang %compile_flags% %compile_shared_library% %linker_flags% %BasePath%\build\android\ak_atomic_test\src\main\cpp\ak_android_atomic_test.c android_native_app_glue.o -u ANativeActivity_onCreate %compile_out% libnative-activity.so
popd